// The standardized incident checklist (§2.2 / annex §7 of the spec).
// 8 fixed categories, 55 fixed items. Codes are stable slugs stored as data
// on `events` rows — once shipped, a code is only ever added to, never
// renumbered or deleted, since historical events reference it by value.
...
export function findReferenceItem(itemCode: string): { category: ReferenceCategory; item: ReferenceItem } | undefined {
  for (const category of REFERENCE_LIST) {
